...
that is, we prove that $\int \chi_{t,E} d\lambda = \int \chi_E d\lambda$, where $\chi_{t,E}(x) = \chi_E(x-t)$.
Notice that $\chi_{t,E}(x) = \chi_E(x-t) = 1$ when $x \in E+t$.
Furthermore, we have that $\int \chi_E d\lambda = \lambda(E) = \lambda(E+t)$, where the last equality follows from 2.7.
Thus, $\int \chi_E d\lambda = \int \chi_{t,E} d\lambda$.
