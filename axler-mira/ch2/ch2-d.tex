...
Finally, notice that $(t + A) \setminus (t + B) = t + (A \setminus B)$.
If $x \in (t + A) \setminus (t + B)$, then $x = t + a$ for some $a \in A$ and $x \neq t + b$ for all $b \in B$.
If $x \in t + (A \setminus B)$, then $x = t + a$, for some  $a \in A$ and $a \notin B$, that is $x - t \neq b$ for all $b \in B$.
Thus, the sets are equivalent because they represent the same elements.

Now, we have $\absoluteValue{(t+A) \setminus (t + B)} = \absoluteValue{t + (A \setminus B)} = \absoluteValue{A \setminus B} = 0$, because the outer measure is translation invariant.
\end{proof} 
...
As in the previous exercise, we will prove $tA \setminus tB = t(A \setminus B)$.
To see this, notice that $x \in tA \setminus tB$ means that $x = ta$ for some $a \in A$ and $x \neq tb$ for all $b \in B$.
Likewise, $x \in t(A \setminus B)$ means that $x = ta$ for $a \in A$ and $a \notin B$, which is the same as $x \neq tb$ for all $b \in B$.
Thus, the same sets are equivalent.

Now, we have $\absoluteValue{tA \setminus tB} = \absoluteValue{t(A \setminus B)} = \absoluteValue{t}\absoluteValue{A \setminus B} = 0$, as required.
\end{proof} 
