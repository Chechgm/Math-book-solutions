...
\end{exercise}
\begin{proof}
We have that $(T - \lambda I)x = (-\lambda x_1, (1 - \lambda) x_2, - \lambda, (1 - \lambda) x_4, \dots)$.
For $\lambda \neq 0$, $\cN(T_\lambda^n) = \set{0}$; 
for $\lambda = 0$, then $\cN(T_\lambda^n)$ would be all the vectors of the form $(x_1, 0, x_3, 0, \dots)$;
finally, for $\lambda = 1$, $\cN(T_\lambda^n)$ are the vectors of the form $(0, x_2, 0, x_4, \dots)$.
...
Another way to conclude that $T$ is not compact, if by noting that $\cN(T_\lambda)$ must be finite dimensional for $\lambda \neq 0$ (8.3-3).
This does not hold, as preivously shown.
\end{proof} 
